...
export const execute = async () => {
    ...
    const REFRESH_FREQUENCY_IN_MS = 2_000; // 2 seconds
    ...
    const marketConfig = Array.from(client.marketConfigs.values()).find((market) => market.name === "SOL/USDC");
    if (!marketConfig) {
        throw new Error("Market config not found");
    }
    // Get the market metadata for the market you wish to trade on
    //const marketState = client.marketStates.get(marketConfig.marketId);
    const marketPubkey = new PublicKey(
        //mainnet "4DoNfFBfF7UokCC2FQzriy7yHK6DY6NVdYpuekQ5pRgg"
        //"CS2H8nbAVVEUHWPF5extCSymqheQdkd4d7thik6eet9N" //devnet
    ...
    do {
        ...
        try {
            ...
            try {
                ...
                counter += 1;    
                console.log("Inner Try Error: ", err);
                continue;
            }
            counter += 1;
